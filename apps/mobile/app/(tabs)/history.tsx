...
import { View, Text, ScrollView, StyleSheet, TouchableOpacity, SectionList, ActivityIndicator } from 'react-native';
  ...
  return (
    <TouchableOpacity style={[
      ...
      <View style={styles.rowMid}>
        <Text style={[styles.rowTopic, { color: theme.fg }]}>{TOPIC_LABELS[session.topic] ?? session.topic}</Text>
    ...
    <View style={[styles.outer, { backgroundColor: theme.bg }]}>
      ...
      {isLoading ? (
        ...
        <View style={styles.emptyWrap}>
          ...
          <Text style={[styles.emptySub, { color: theme.fgMuted }]}>Your completed interviews will appear here</Text>
        </View>
      ) : (
        <SectionList
          sections={groups}
          ...
          renderSectionHeader={({ section }) => (
            <View style={styles.sectionHeaderWrap}>
              <Text style={[styles.sectionHeaderText, { color: theme.fgMuted }]}>{section.label}</Text>
            </View>
          )}
          renderItem={({ item, index, section }) => (
            <View style={index === 0 ? styles.groupWrapStart : (index === section.data.length - 1 ? styles.groupWrapEnd : undefined)}>
              {index === 0 && <View style={[styles.groupOuter, { backgroundColor: theme.surface, borderColor: theme.border }]} />}
              <SessionRow session={item} last={index === section.data.length - 1} />
              {index === section.data.length - 1 && <View style={[styles.groupBottom, { backgroundColor: theme.surface, borderColor: theme.border }]} />}
            </View>
          )}
...
const styles = StyleSheet.create({
  ...
  title: { fontFamily: Font.bold, fontSize: 22, paddingHorizontal: 16, paddingVertical: 4, marginBottom: 8 },
  ...
  sectionHeaderText: { fontFamily: Font.semiBold, fontSize: 12, textTransform: 'uppercase', letterSpacing: 0.72 },
  groupOuter: { marginHorizontal: 16, borderTopLeftRadius: 12, borderTopRightRadius: 12, borderWidth: 1, borderBottomWidth: 0 },
  groupWrapStart: {},
  groupWrapEnd: {},
  groupBottom: { marginHorizontal: 16, borderBottomLeftRadius: 12, borderBottomRightRadius: 12, borderWidth: 1, borderTopWidth: 0, height: 1 },
  row: { flexDirection: 'row', alignItems: 'center', gap: 12, padding: 13, paddingHorizontal: 14, marginHorizontal: 16 },
  rowIcon: { width: 38, height: 38, borderRadius: 9, alignItems: 'center', justifyContent: 'center', flexShrink: 0 },
