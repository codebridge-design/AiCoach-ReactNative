...
export default function ResultScreen() {
  ...
  const { avg, scores: scoresRaw, seconds, topic, answered, delta: deltaRaw } = useLocalSearchParams<{
    avg: string; scores: string; seconds: string; topic: string; answered: string; delta: string;
  }>();
  ...
  return (
    <View style={[styles.outer, { backgroundColor: theme.bg }]}>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent}>
        {/* Readiness delta */}
        <Card padding={16} style={[styles.deltaCard, { backgroundColor: theme.greenSoft, borderColor: 'transparent' }]}>
          <View style={styles.deltaRow}>
            ...
            <View style={{ flex: 1 }}>
              ...
              <Text style={[styles.deltaSub, { color: theme.fgMuted }]}>Keep your streak going 🔥</Text>
            </View>
          </View>
        </Card>

        <Text style={[styles.breakdownLabel, { color: theme.fgMuted }]}>PER QUESTION</Text>
        <View style={[styles.breakdownList, { backgroundColor: theme.surface, borderColor: theme.border }]}>
          {scores.map((s, i) => {
            const c = s != null ? scoreColor(s) : theme.fgMuted;
            return (
              <View key={i} style={[styles.breakdownRow, i < scores.length - 1 && { borderBottomWidth: 1, borderBottomColor: theme.border }]}>
...
const styles = StyleSheet.create({
  outer: { flex: 1 },
  header: { backgroundColor: '#1B448B', paddingTop: 56, paddingBottom: 32, paddingHorizontal: 20, alignItems: 'center' },
  ...
  deltaIcon: { width: 44, height: 44, borderRadius: 11, alignItems: 'center', justifyContent: 'center', flexShrink: 0 },
  deltaTitle: { fontFamily: Font.bold, fontSize: 15 },
  deltaSub: { fontFamily: Font.regular, fontSize: 12.5, marginTop: 1 },
  breakdownLabel: { fontFamily: Font.bold, fontSize: 11, textTransform: 'uppercase', letterSpacing: 0.66, marginBottom: 10, marginTop: 6 },
  ...
  qNum: { width: 26, height: 26, borderRadius: 7, alignItems: 'center', justifyContent: 'center', flexShrink: 0 },
