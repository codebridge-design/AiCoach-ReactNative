...
export async function listSessions(req: any, res: Response) {
...
export async function endSession(req: any, res: Response) {
  ...
  const { data: todayPrev } = await supabase
    ...
    .eq('user_id', req.userId).eq('status', 'completed')
  ...
  if (!todayPrev?.length) {
    ...
    const { data: prof } = await supabase.from('profiles').select('streak_count').eq('id', req.userId).single();
...
export async function deleteSession(req: any, res: Response) {
