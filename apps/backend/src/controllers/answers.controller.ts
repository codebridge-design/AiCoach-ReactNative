...
export async function submitAnswer(req: any, res: Response) {
...
export async function submitVoiceAnswer(req: any, res: Response) {
  if (!req.file) { res.status(400).json({ error: 'Audio file required' }); return; }
  const { session_id, question_id } = req.body;

  const { data: session } = await supabase.from('sessions').select('level').eq('id', session_id).eq('user_id', req.userId).single();
  if (!session) { res.status(404).json({ error: 'Session not found' }); return; }

  const { data: question } = await supabase.from('questions').select('text, category').eq('id', question_id).eq('session_id', session_id).single();
  if (!question) { res.status(404).json({ error: 'Question not found' }); return; }
...
export async function editAnswer(req: any, res: Response) {
...
export async function skipAnswer(req: any, res: Response) {
